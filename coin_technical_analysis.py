...
def process_symbol(symbol):
    try:
        ...
        # ✅ slope calculations RSI SLOPE
        short_slope = rsi_closed - rsi_prev
        long_slope  = rsi_closed - rsi.iloc[-5]

        # ✅ RSI direction condition
        if not (rsi_prev < rsi_closed < 48):
            return None

        # ✅ slope filter (IMPORTANT ADD)
        if short_slope <= 0.5 or long_slope < 3:
            return None
        ...
        # ✅ ensure breakout is happening NOW
