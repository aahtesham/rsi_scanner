...
def get_all_usdt_symbols():
    logger.info("Fetching all USDT trading symbols from Binance")
...
def scan():
    ...
    for idx, symbol in enumerate(symbols, start=1):
        ...
        try:
            ...
            rsi_now   = rsi.iloc[-2]   # last CLOSED candle  ✅
    ...
    if matches:
        ...
        with open("results.txt", "a") as f:
            f.write(f"\n=== RSI 53 →  MATCHES ===\n")
            f.write(f"Scan Time: {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}\n")
            f.write("-" * 45 + "\n")
            for m in matches:
                f.write(f"{m['symbol']:<10} | RSI: {m['rsi']}\n")
            f.write("-" * 45 + "\n")
        ...
        with open("results.txt", "a") as f:
            f.write(f"No matching tokens found at {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}\n\n")
...
if __name__ == "__main__":
    logger.info("RSI Scanner started")
